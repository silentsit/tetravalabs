...
| `/tools/peptide-identity` | Peptide identity index \| Tetrava | Peptide identity index for research peptides: sequence, CAS, mass, and expected MS ions. Compare two names to see if they are the same molecule. RUO. | Organization, WebSite, WebPage, BreadcrumbList | Query params `a`, `b`, `obs` are shareable; canonical stays the hub |
