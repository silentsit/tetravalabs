...
| `/{handle}` | {productName with strengths} — {category} \| Tetrava Labs | {productName with strengths} for laboratory research (RUO). {purity} purity with lot-linked COA. [CAS …]. | Organization, WebSite, Product, WebPage, BreadcrumbList, FAQPage | One URL per compound. Multi-strength → e.g. MOTS-c (5mg / 10mg / 20mg / 40mg). Legacy `/{handle}-{strength}` redirects to parent. Title/description are compound-level (not per selected strength). H1 may show the selected strength for UX. |
| `/buy-bpc-157-online` | BPC-157 Peptide for Sale • Tissue Repair \| Tetrava Labs | BPC-157 peptide for sale at Tetrava Labs — third-party verified 99%+ purity with lot-linked COA. Buy BPC-157 online with us today. | Organization, WebSite, Product, WebPage, BreadcrumbList, FAQPage | Curated override in `product-seo-overrides.ts`; image alt: BPC-157 peptide for sale in vial |
