...
| `/{handle}` | {productName with strengths} — {category} \| Tetrava Labs | {productName with strengths} for laboratory research (RUO). {purity} purity with lot-linked COA. [CAS …]. | Organization, WebSite, Product, WebPage, BreadcrumbList, FAQPage | Multi-strength → e.g. BPC-157 (5mg / 10mg); Product offers aggregate all strengths |
