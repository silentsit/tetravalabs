...
- Always inject: `Organization` (incl. `PostalAddress`: 455 Gateway Drive, Pacifica, CA 94044, US; `hasMerchantReturnPolicy` → `/refund`), `WebSite` (SearchAction → `/search?q=…`)
...
1. **`/reorder/{token}`** — inherits root title/description; not marked noIndex  
2. **`not-found`** — no dedicated metadata / noindex  
